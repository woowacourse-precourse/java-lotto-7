...
- [x] | `구입 금액` 입력
- [ ] | 입력된 `구입 금액` 을 객체로 저장
- [ ] | 입력된 `구입 금액`이 1,000원으로 나누어 떨어지는지 검증
- [ ] | 잘못된 입력에 대한 [ERROR] 메시지 표시 및 재입력
- [ ] | `구입 금액`에 해당하는 `로또` 생성
...
- 잘못된 입력에 대한 IllegalArgumentException 예외 처리한다
    ...
    - [ ] | 입력값이 빈칸이거나 공백인 경우
    - [ ] | 입력값이 양의 정수가 아닌 경우
    - [ ] | 입력값의 중간에 공백이 있을 경우 (ex. "100 0","1 1,2")
        - 이때, 입력값이 앞뒤로 공백이 있는 경우는 예외 처리하지 않는다. (ex. "1000 ", "1 , 2 , 3")<br>
          ...
          하지만 위와 같이 "100 0"와 "1 1,2"와 같이 의미적으로 잘못 판단 될 수 있는 경우는 예외 처리가 되어야한다.<br>
