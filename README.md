...
TicketResult ticket = GetLottoTicketUsecase.execute(Long ticketId);

OutputView - {ticket.getCount}개를 구매했습니다.
OutputView - {ticket.getAll}
```
...
```java
Ticket ticket = Ticket.of(LottoCount lottoCount);
...
- TicketService - 조회하라
```java
Ticket ticket = TicketRepository.getById(Long ticketId);
```
