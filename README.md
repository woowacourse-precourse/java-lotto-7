...
#### **1-3. 구입 금액 검증 기능**
- [ ] 입력된 금액이 **정수**인지 확인한다.
    - [ ] 정수가 아닌 값이 입력되면 `IllegalArgumentException`을 발생시킨다.
    - [ ] 예외 메시지: `[ERROR] 구입 금액은 숫자여야 합니다.`
- [ ] 입력된 금액이 **1000원 이하**인 경우 `IllegalArgumentException`를 발생시킨다.
    - [ ] 예외 메시지: `[ERROR] 구입 금액은 1000원 이상이어야 합니다.`
- [ ] 입력된 금액이 **1,000원 단위**인지 확인한다.
    - [ ] 1,000원 단위가 아니면 `IllegalArgumentException`을 발생시킨다.
...
#### **2-1. 로또 장수 계산 기능**
- [ ] **유효한 금액**을 입력받아 발행할 로또 장수를 계산한다.
    - 예: 8,000원 → 8장 발행.

#### **2-2. 로또 발행 기능**
- [ ] `Randoms.pickUniqueNumbersInRange(1, 45, 6)`을 사용해 로또 번호를 생성한다.
    - [ ] 생성된 번호를 **오름차순으로 정렬**한다.
    - [ ] 발행한 번호 중 중복된 조합이 없는지 확인한다.
    - [ ] 중복된 조합이 있을 경우 다시 번호를 생성한다.

#### **2-3. 로또 발행 결과 출력 기능**
- [ ] **"{N}개를 구매했습니다."** 메시지를 출력한다.
    - 예: **"8개를 구매했습니다."**
- [ ] 각 로또 번호를 한 줄씩 출력한다.
    - 예:
      ```
...
#### **3-2. 당첨 번호 입력 기능**
- [ ] 사용자가 쉼표(,)로 구분된 **6개의 번호**를 입력한다.

#### **3-3. 당첨 번호 검증 기능**
- [ ] **입력된 번호의 개수**가 정확히 6개인지 확인한다.
    - [ ] 부족하거나 초과할 경우 `IllegalArgumentException` 발생.
    - [ ] 예외 메시지: `[ERROR] 당첨 번호는 6개여야 합니다.`
- [ ] **각 번호가 1~45 범위**에 있는지 확인한다.
    - [ ] 범위를 벗어난 경우 `IllegalArgumentException` 발생.
    - [ ] 예외 메시지: `[ERROR] 당첨 번호는 1부터 45 사이의 숫자여야 합니다.`
- [ ] **숫자가 아닌 문자**가 포함된 경우 `IllegalArgumentException` 발생.
    - [ ] 예외 메시지: `[ERROR] 당첨 번호는 숫자여야 합니다.`
- [ ] **중복된 번호가 없는지** 확인한다.
    - [ ] 중복된 번호가 있을 경우 `IllegalArgumentException` 발생.
...
- [ ] **구매한 모든 로또**와 당첨 번호를 비교한다.
- [ ] **일치하는 번호의 개수**를 계산한다.

#### **5-2. 2등 판정 기능**
- [ ] **5개 번호가 일치**하고 **보너스 번호가 포함**된 경우 **2등**으로 처리한다.

#### **5-3. 등수 판정 기능**
- [ ] 번호 일치 개수에 따라 **1등부터 5등까지** 등수를 판정한다.
...
- [ ] **"당첨 통계"** 메시지와 구분선(`---`)을 출력한다.

#### **6-2. 등수별 당첨 결과 출력 기능**
- [ ] 각 등수의 **당첨 횟수와 상금**을 출력한다.
    - 예:
      ```
      3개 일치 (5,000원) - 1개
      4개 일치 (50,000원) - 0개
...
#### **6-3. 총 수익 계산 기능**
- [ ] 각 등수별 **(당첨 횟수 × 상금)** 으로 총 수익을 계산한다.

#### **6-4. 수익률 계산 및 출력 기능**
- [ ] **수익률**을 `(총 수익 / 총 구입 금액) × 100`으로 계산한다.
...
- [ ] 잘못된 입력이 발생하면 `[ERROR]`로 시작하는 에러 메시지를 출력한다.

#### **7-2. 재입력 기능**
- [ ] 에러 발생 시 해당 입력을 **다시 요청**한다.
