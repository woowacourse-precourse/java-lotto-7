...
#### Model
-[x] 각 랭크 로또가 하나씩 주어지면 결과값을 하나씩 반환한다.
-[x] 로또가 없을 때 0을 반환한다.
-[x] 각 랭크 로또가 하나씩 주어지면 수익률을 반환한다.
-[x] 만원 입력시 로또를 10장 살 수 있다.
-[x] 천 단위로 나누어지지 않은 경우에는 예외를 발생시킨다.
-[x] 자료형 범위를 넘는 경우에는 예외를 발생시킨다.
-[x] 모든 번호가 일치하면 제일 높은 등급의 로또 결과가 생성된다.
-[x] 보너스 번호 포함 6개의 숫자가 포함됐을 떄를 확인한다.
-[x] 보너스 번호 없이 5개만 일치했을 때를 확인한다.
-[x] 로또가 개수에 맞게 잘 생성되는지 확인한다.
-[x] 랭크가 매겨진 로또 결과가 정상적으로 생성되는지 확인한다.
-[x] 로또가 정상적으로 생성되어 저장되는지 확인한다.
-[x] 생성자가 제대로 작동되어 당첨번호를 생성한다.
-[x] 중복되는 번호가 있으면 예외를 발생시킨다.

#### Validator
-[x] 음수 입력 테스트 (구매 금액, 당첨 번호, 보너스 번호)
-[x] 공백 입력 테스트 (구매 금액, 당첨 번호, 보너스 번호)
-[x] 0 입력 테스트 (구매 금액, 당첨 번호, 보너스 번호)
-[x] 문자 입력 테스트 (구매 금액, 당첨 번호, 보너스 번호)
-[x] null 입력 테스트 (구매 금액, 당첨 번호, 보너스 번호)
-[x] 보너스 번호가 로또 번호 범위를 벗어났을 때 입력 테스트 (당첨 번호, 보너스 번호)
-- -- --
## 다이어그램
#### 클래스 다이어그램 (설계 기준)
