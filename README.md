...
- [ ] 애플리케이션이 시작되면, **로또 구입 금액**을 **입력**받는다.
  - 입력은 `Console.readLine()`을 통해 입력받는다.
  - 구입 금액은 **`1000`원 단위**로 입력받으며, 아닐 경우 **예외 처리**한다.
...
### 1. 손님이 로또를 구매를 위해 금액을 지불한다.
- [ ] 구매 금액 입력
  - [ ] `구입 금액을 입력해 주세요.`라는 문구를 출력
  - [ ] 구입 금액을 `Console.readLine()`메서드를 활용하여 입력받음
- [ ] 구매 금액 입력 유효성 검사
  - [ ] 입력된 구입 금액이 `1,000원` 단위의 정상 입력인지 검사
  - [ ] 입력이 올바르지 않다면, 구매 금액 입력부터 다시 과정을 시작

### 2. 상점은 받은 구매 금액 만큼의 로또를 발급한다.
- [ ] 로또 번호 발급
  - [ ] 발급하기로 한 하나의 로또 당 **중복되지 않는** 6개의 숫자 발급
  - [ ] 로또의 랜덤 번호는 `Randoms.pickUniqueNumbersInRange()`를 활용하여 발급
  - [ ] 6개의 숫자를 가지고 로또 객체의 생성을 요청
- [ ] 로또 객체 생성 및 저장
  - [ ] 발급된 6개의 숫자를 기반으로, `Lotto` 객체를 생성
  - [ ] 생성된 객체들을 `Repository`에 저장
...
- `NumberFormatException`인 경우 (문자열을 숫자로 변환할 때 잘못된 입력일 경우)
  - [ ] 구매 금액을 입력할 때, 숫자가 아닌 내용이 입력될 수 없다.
...
- `IllegalArgumentException`인 경우 (메서드가 매개변수로 잘못된 값이 전달되었을 때 발생) 
  ...
  - [ ] 구매 금액의 단위는 `1,000`원 단위로 입력되어야 한다.(0도 허용하지 않는다)
...
- `IllegalStateException`인 경우 (객체의 상태가 메서드를 호출하기에 적절하지 않을 때 발생)
  - 구매 금액을 입력할 때, 당첨 번호를 입력하려는 경우(쉼표로 구분된 6개의 숫자)
  - 구매 금액을 입력할 때, 보너스 번호를 입력하려는 경우(1부터 45 사이 하나의 숫자)
  - 당첨 번호를 입력할 때, 구매 금액을 입력하려는 경우(1,000 단위의 입력)
  - 당첨 번호를 입력할 때, 보너스 번호를 입력하려는 경우(1부터 45 사이 하나의 숫자)
  - 보너스 번호를 입력할 대, 구매 금액을 입력하려는 경우(1,000 단위의 입력)
  - 보너스 번호를 입력할 때, 6개의 숫자를 입력하려는 경우(쉼표로 구분된 6개의 숫자)
