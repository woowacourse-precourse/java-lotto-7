...
- 구입 금액을 입력 받는다.
    - 입력 안내 문구를 출력한다: “구입금액을 입력해 주세요.”
    - 입력 값은 유효해야 한다.
        - 숫자만 입력해야 한다.
        - 금액 범위는 1,000~10,000이다.
        - 1,000으로 나누어 떨어져야 한다.
    - 입력 값을 저장한다.
        - 구입 금액을 저장한다.
        - 1,000으로 나눈 몫만큼 구입한 개수를 저장한다.
    - 구입한 개수를 출력한다: “n개를 구매했습니다.”

- 로또를 발행한다.
    - 중복되지 않는 6개의 랜덤한 값을 구한다.
    - Random 값 추출은 `camp.nextstep.edu.missionutils.Randoms`의 `pickUniqueNumbersInRange()`를 활용한다.
    - Lotto 클래스에 숫자를 저장한다.
        - 숫자는 오름차순으로 정렬한다.
    - 발행 결과를 출력한다: “[n, n, n, n, n, n]”
    - 구입한 개수만큼 발행을 반복한다.
    - Lotto 클래스는 ArrayList로 관리한다.

- 당첨 번호를 입력 받는다.
    - 입력 안내 문구를 출력한다: “당첨 번호를 입력해 주세요.”
...
- 예외 상황을 처리한다.
    ...
    - 에러 문구를 출력한다: “[ERROR] 에러 메시지”
    - 입력이 중단된 부분부터 입력을 다시 받는다.
...
- 추첨 결과를 출력한다.
    ...
    - 당첨 내역을 출력한다: “n개 일치[, 보너스 볼 일치] (n원) - n개”
