...
- [x] **사용자가 구매한 각 로또 번호에 대해 당첨 여부를 판단한다.**
    - [x] 각 로또 번호와 입력된 당첨 번호 6개를 비교하여 **일치하는 숫자의 개수를 계산**한다.
    - [x] 보너스 번호를 확인하여 **보너스 번호가 일치하는지** 판단한다.
    - [x] **당첨 기준을 충족하는 등수를 확인**한다.
        - [x] 1등: **6개 번호 모두 일치**
        ...
        - [x] 5등: **3개 번호 일치**
        - [x] 그 외: **당첨되지 않음**
    - [x] **각 등수별 당첨된 로또 개수를 기록**한다.

### 당첨 통계 출력

- [x] **당첨 통계 출력 안내 멘트를 출력한다.**
    - [x] "**당첨 통계**" 안내 문구를 출력한다.
    - [x] **"---"**(구분선)을 출력한다.

- [x] **등수별 당첨 개수를 출력한다.**
    - [x] **1등부터 5등까지 등수별 당첨 개수를 출력한다.**
        - [x] **등수별 당첨 금액과 일치하는 개수**를 출력 형식에 맞게 출력한다.
        - 예시:
            ```
            3개 일치 (5,000원) - 1개
            4개 일치 (50,000원) - 0개
...
- [x] **출력 형식이 일관되도록 주의하여 출력한다.**
    - [x] 금액은 세 자리마다 콤마(,)를 포함해 출력한다.
    - [x] 당첨되지 않은 등수는 **"0개"**로 출력한다.

### 총 수익률 출력

- [x] **총 수익률을 계산한다.**
    - [x] **구매한 총 금액을 기준으로 수익률을 계산**한다.
        - [x] **수익률 = (총 당첨 금액 / 총 구입 금액) * 100**으로 계산한다.
    - [ ] 계산된 수익률을 **소수점 둘째 자리에서 반올림**하여 표시한다.
    - [x] **수익률이 100% 이상인 경우에도 정확히 계산된 값을 출력한다.**

- [x] **총 수익률을 출력한다.**
    - [x] 출력 형식에 맞게 "**총 수익률은 XX.X%입니다.**"와 같은 형태로 출력한다.
    - 예시:
        ```
        총 수익률은 62.5%입니다.
        ```
...
- [x] Lotto에 numbers 이외의 필드(인스턴스 변수) 추가 X
- [x] Lotto의 패키지 변경은 가능
