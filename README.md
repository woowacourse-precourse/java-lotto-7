...
- [ ] 구입 금액은 1000원 단위로 입력된다.
    - [ ] 구입 금액 입력이 숫자가 아니면 에러 메세지를 출력하고 다시 입력받는다.
    - [ ] 구입 급액 입력이 1000원 단위가 아니면 에러 메세지를 출력하고 다시 입력받는다.
- [ ] 구입 갯수를 출력 한다.
    - [ ] 구입 갯수 = { 구입 금액 / 1000 }
- [ ] 로또를 발행 한다.
    - [ ] 로또 발행 시 1~45 범위의 고유 숫자를 6개 선정한다.
    - [ ] 로또 번호가 6개가 아니면 예외를 발생시킨다.
    - [ ] 로또 번호가 1~45 범위가 아니면 예외를 발생시킨다.
    - [ ] 로또 번혹가 중복되면 예외를 발생시킨다.
    - [ ] 로또 발행에 랜덤을 적용한다.
- [ ] 발행된 로또들을 출력한다.
    - [ ] 로또 출력은 한 줄에 로또 하나씩 출력한다.
    - [ ] 로또 하나 출력은 []로 묶고 숫자 사이에 ", "를 입력한다.
- [ ] 당첨 번호 추첨은 1~45 범위의 고유 숫자 6개와 (,)로 구분한다.
    - [ ] 당첨 번호 입력이 숫자,숫자,숫자,숫자,숫자,숫자 패턴이 아니면 에러 메세지를 출력하고 다시 입력받는다.
    - [ ] 당첨 번호 입력에 중복이 있으면 에러 메세지를 출력하고 다시 입력받는다.
    - [ ] 1~9까지의 입력을 01~09로 입력해도 정상적으로 처리한다.
- [ ] 보너스 번호 추첨은 1~45 범위의 당첨 번호와 중복 되지 않는 양수 숫자가 입력 되어야 한다.
    - [ ] 보너스 번호 입력이 1~45 범위가 아니면 에러 메세지를 출력하고 다시 입력받는다.
    - [ ] 보너스 번호 입력이 당첨 번호 입력과 중복되면 에러 메세지를 출력하고 다시 입력받는다.
- [ ] 담첨 통계를 계산한다.
    - [ ] 3개 일치 : 가중치 5
    - [ ] 4개 일치 : 가중치 50
    - [ ] 5개 일치 : 가중치 1500
    - [ ] 5개 일치 + 보너스 번호 일치 : 가중치 2,000
    - [ ] 6개 일치 : 가중치 2,000,000
