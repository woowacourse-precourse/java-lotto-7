...
## 프로젝트 구조
- **src/main/java/lotto**
    - `Application.java`: 프로그램 실행의 시작점으로, `main()` 메서드를 통해 로또 게임을 시작한다.
    - `Lotto.java`: 로또 번호를 관리하는 클래스이다. 로또 번호의 유효성을 검사하고 저장한다.
    - `LottoGame.java`: 로또 게임의 전반적인 흐름을 관리하는 클래스이다. 로또 번호 생성, 당첨 번호 및 보너스 번호 입력 등을 처리한다.
    - `LottoRank.java`: 로또 당첨 등급을 관리하는 열거형이다. 각 등급에 대한 일치 개수와 상금을 정의한다.
    - `LottoStatistics.java`: 로또 게임의 결과를 집계하고 수익률을 계산하는 클래스이다.

- **src/test/java/lotto**
    - `ApplicationTest.java`: Application 클래스에 대한 통합 테스트를 진행하는 클래스이다. 프로그램의 전체 흐름을 테스트한다.
    - `LottoGameTest.java`: LottoGame 클래스의 개별 메서드와 주요 기능을 검증하는 단위 테스트 클래스이다.
    - `LottoTest.java`: Lotto 클래스의 로또 번호 생성 및 유효성 검증을 테스트하는 클래스이다.
...
**1. ApplicationTest**
- 기능 테스트
    - 사용자가 8,000원을 입력하여 8개의 로또를 구매했을 때, 각 로또 번호와 당첨 내역 및 총 수익률이 올바르게 출력되는지 검증한다.
- 예외 테스트
    - 잘못된 입력(1000j)에 대해 프로그램이 [ERROR] 메시지를 출력하며 예외를 처리하는지 검증한다.

**2. LottoGameTest**
- 구입 금액 유효성 검사
    - 구입 금액이 1,000원 단위로 입력되지 않을 때 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 로또 번호 생성 테스트
    - 로또 번호가 올바르게 생성되고, 각 로또가 6개의 중복되지 않는 1부터 45 사이의 숫자로 구성되어 있는지 확인한다.
- 당첨 번호 구분자 예외
    - 쉼표(,)로 구분되지 않은 입력이 들어올 때 예외가 발생하는지 확인한다.
- 유효하지 않은 문자 예외
    - 입력된 로또 번호에 숫자가 아닌 문자가 포함되어 있을 때 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 보너스 번호 중복 예외
    - 보너스 번호가 당첨 번호와 중복될 경우 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 보너스 번호 범위 예외
    - 보너스 번호가 1부터 45 범위를 벗어날 경우 예외가 발생하는지 확인한다.
- 보너스 번호 공백 예외
    - 보너스 번호가 공백일 때 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 보너스 번호 여러 값 입력 예외
    - 보너스 번호가 여러 값으로 입력될 때 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 정상적인 로또 게임 실행
    - 정상적인 당첨 번호와 보너스 번호를 입력할 때 예외가 발생하지 않는지 확인한다.

**3. LottoTest**
- 로또 번호 개수 초과 예외
    - 로또 번호가 6개보다 많을 경우 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 로또 번호 개수 부족 예외
    - 로또 번호가 6개보다 적을 경우 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 로또 번호 중복 예외
    - 로또 번호에 중복된 숫자가 포함될 경우 `IllegalArgumentException` 예외가 발생하는지 확인한다.
- 로또 번호 범위 예외
    - 로또 번호가 1부터 45 범위를 벗어날 경우 예외가 발생하는지 확인한다.
- 정상적인 로또 번호 생성
    - 1부터 45 사이의 중복되지 않는 6개의 숫자로 구성된 로또 번호가 올바르게 생성되는지 확인한다.
