...
├── controller
│   └── LottoController.java : 프로그램 전체 흐름 조절
├── domain
│   ├── BonusNumber.java : 보너스 번호 검증, 보너스 번호 일치 여부 확인
│   ├── Lotto.java : 로또 번호 검증, 번호 포함 여부 확인, 오름차순 정렬 문자열 반환
│   ├── LottoMachine.java : 로또 발행
│   ├── LottoNumberGenerator.java : 로또 번호 생성
│   ├── Lottos.java : 로또 리스트 관리
│   ├── Money.java : 구입 금액 검증, 구매 수량 반환
│   ├── Profit.java : 수익률 계산, 수익률 포맷 반환
│   ├── Rank.java : 순위별 당첨 금액, 출력 메시지 관리
│   ├── WinningLotto.java : 당첨 번호 검증, 로또와의 일치 개수 확인
│   └── WinningResult.java : 당첨 통계 계산, 통계 문구 반환, 총 당첨 금액 반환
├── exception
│   ├── InputNumberFormatException.java : 숫자 입력값 예외
│   ├── lotto
│   │   ├── LottoInputFormatException.java : 로또 포맷 예외
│   │   ├── LottoNumberDuplicateException.java : 로또 중복 예외 
│   │   ├── LottoNumberRangeException.java : 로또 범위 예외
│   │   └── LottoNumberSizeException.java : 로또 크기 예외
│   └── money
│       ├── DivideMoneyException.java : 구입 금액 나누기 예외
│       ├── MaximunMoneyException.java : 구입 금액 최대값 예외
│       └── ZeroMoneyException.java : 구입 금액 0원 예외
├── util
│   ├── ErrorMessage.java : 범용 에러 메시지
│   ├── Limit.java : 범용 숫자 메시지
│   └── Parser.java : 파싱
└── view
...
3. 구입 금액, 보너스 번호가 공백으로 입력되면 예외처리한다.
