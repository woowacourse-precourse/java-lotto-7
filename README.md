...
- [ ] 사용자에게 로또 구입 금액을 입력받는다.
    ...
    - [ ] 입력된 구입 금액이 `1000원 미만`인지 확인한다.
        - [ ] `1000원 미만`인 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 구입 금액은 최소 1000원 이상이어야 합니다. 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] 입력된 구입 금액이 `1000원 이상인데, 1000원으로 나누어 떨어지는지` 확인한다.
        - [ ] `1000원 이상인데 1000원으로 나누어 떨어지지 않는` 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 구입 금액은 1000원 단위로 입력해야 합니다. 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] 입력된 구입 금액이 `음수`인지 확인한다.
        - [ ] `음수`인 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 구입 금액은 음수일 수 없습니다. 1000원 이상의 금액을 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] 입력된 구입 금액이 `0원`인지 확인한다.
        - [ ] `0원`인 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 구입 금액은 0원일 수 없습니다. 1000원 이상의 금액을 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] 입력된 구입 금액이 `숫자`인지 확인한다.
        - [ ] `숫자가 아닌` 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 구입 금액은 숫자여야 합니다. 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] 입력된 구입 금액이 `100000원 초과`인지 확인한다.
        - [ ] `100000원 초과`인 경우 `IllegalArgumentException`을 발생시키고 **"[ERROR] 로또는 1인당 최대 100장까지 구매할 수 있습니다. 다시 입력해 주세요."** 라는 오류 메시지를 출력한 후 다시 입력을 받는다.
    - [ ] **그외 경우 다음 단계를 진행한다.**

### 로또 발급 진행

- [ ] **로또 발급을 시작한다.**
    - [ ] 사용자가 입력한 `구입 금액에 따라 발행할 로또 수량을 계산한다.`
        - [ ] 구입 금액을 1,000원으로 나눈 값을 통해 로또 수량을 계산한다.
        - [ ] 계산된 로또 수량을 사용자에게 출력한다.
            - 예시: **"8개를 구매했습니다."**

- [ ] **로또 번호를 발행한다(구매 수량만큼 반복)** 
    - [ ] 각 로또에 대해 **1~45 범위의 중복되지 않는 6개의 숫자를 랜덤하게 선택**한다.
    - [ ] **선택된 숫자를 오름차순으로 정렬**하여 로또 번호를 구성한다.
    - [ ] 발행한 로또 번호를 출력 형식에 맞게 출력한다.
        - 예시: **"[8, 21, 23, 41, 42, 43]"**

- [ ] **발행한 모든 로또 번호를 사용자에게 출력한다.**
    - [ ] 발행한 로또 수량만큼의 번호를 오름차순으로 출력한다.
    - [ ] 예시 출력 형식을 따른다.
        - 예시:
            ```
            8개를 구매했습니다.
