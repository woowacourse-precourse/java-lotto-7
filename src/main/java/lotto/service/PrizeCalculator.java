...
public class PrizeCalculator {
    private static final Map<Integer, Integer> prizeAmounts = Map.of(
            5, 5_000,
            4, 50_000,
            3, 1_500_000,
            2, 30_000_000,
    ...
    public static double calculateProfitRate(Map<Integer, Integer> prizeCounts, UserMoney userMoney) {
        int totalPrize = prizeCounts.entrySet().stream()
                .mapToInt(entry -> prizeAmounts.get(entry.getKey()) * entry.getValue())
                .sum();
    ...
    public static int getPrizeAmount(int rank) {
