...
public class BuyAmount {
    ...
    public BuyAmount(String buyAmountString) {
        ...
        remainValidation(buyAmountString);
    ...
    private void emptyValidation(String buyAmount) {
        if (buyAmount.isEmpty()) {
            throw new IllegalArgumentException("[ERROR] 구입금액은 빈 값이 들어올 수 없습니다.");
        }
    }
    ...
    private void underMinimumAmountValidation(String buyAmount) {
        if (Integer.parseInt(buyAmount) < 1000) {
            throw new IllegalArgumentException("[ERROR] 구매금액은 1000원 이상이어야 합니다.");
        }
    }

    private void remainValidation(String buyAmount) {
