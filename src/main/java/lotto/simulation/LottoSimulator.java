...
public class LottoSimulator {
    public void run() {
        ...
        game.outLottoRankCountInfo(); // 당첨 통계 출력
    ...
    private BonusNumber getBonusNumber(WinningNumber winningNumber) {
        while (true) {
            try {
                ConsoleOutputHandler.bonusNumberMessage(); // 보너스 번호 입력 안내 메세지 출력
                String bonusNumberString = ConsoleInputHandler.input(); // 보너스 번호 문자열로 입력받음
                return new BonusNumber(bonusNumberString, winningNumber);
