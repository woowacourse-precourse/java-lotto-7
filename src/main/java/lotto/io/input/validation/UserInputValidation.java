...
public class UserInputValidation {
	...
	private static final Pattern winningNumberRangePattern =
		Pattern.compile("^[1-9]$|^[0]*[1-9]$|^[1-3]{1}[0-9]{1}$|^4{1}[0-5]{1}$");
	...
	private void validateWinningNumberRange(String[] splitWinningNumbers) {
		boolean hasWrongWinningNumber = Arrays.stream(splitWinningNumbers)
			.map(String::trim)
			.anyMatch(winningNumber -> !winningNumberRangePattern.matcher(winningNumber).matches());
