...
public class LottoWinningResultOutputBuilder {
    ...
    public static String build(Map<WinningLottoRank, Integer> winningCount, Double rateOfReturn) {
        ...
        stringBuilder.append(NEW_LINE).append(LOTTO_WINNING_STAT.getMessage()).append(NEW_LINE);
        ...
        stringBuilder.append(formatRateOfReturn(RATE_OF_RETURN.getMessage(), rateOfReturn));
    ...
    private static String getWinningResultMessage(WinningLottoRank winningLottoRank) {
        if (winningLottoRank == SECOND_PLACE) {
            return SECOND_PLACE_RESULT.getMessage();
        }
        return OTHER_PLACE_RESULT.getMessage();
    }
