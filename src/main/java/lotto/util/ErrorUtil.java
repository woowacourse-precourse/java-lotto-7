...
public enum ErrorUtil {
    PURCHASE_MONEY_NUMBER_ERROR_MESSAGE(
            String.format("%s원 이상의 숫자로 구성된 금액을 입력해주세요.", NumberUtil.LOTTO_PER_PURCHASE_UNIT)),
    ...
    WINNING_LOTTO_RANGE_ERROR_MESSAGE(
            String.format("%s이상 %s이하의 숫자 %s개를 입력해주세요", NumberUtil.MINIMUM_RANGE_NUMBER, NumberUtil.MAX_RANGE_NUMBER,
                    NumberUtil.MAX_PICK_NUMBER)),
    WINNING_LOTTO_REPEAT_ERROR_MESSAGE("중복되지 않는 숫자 6개를 입력해주세요"),
    BONUS_NUMBER_RANGE_ERROR_MESSAGE(
            String.format("%s이상 %s이하의 1개의 숫자를 입력해주세요", NumberUtil.MINIMUM_RANGE_NUMBER, NumberUtil.MAX_RANGE_NUMBER)),
