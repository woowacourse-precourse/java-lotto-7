...
public class LottoManager {
    ...
    public HashMap<LottoRank, Integer> getWinningResult(ArrayList<Lotto> lottos){
        ...
        for(Lotto lotto : lottos){
            ...
            if(count==3){
                winningResult.put(LottoRank.FIFTH, winningResult.getOrDefault(LottoRank.FIFTH, 0)+1);
            }
            else if(count==4){
                winningResult.put(LottoRank.FOURTH, winningResult.getOrDefault(LottoRank.FOURTH, 0)+1);
            }
            else if(count==5 && bonusMatch){
                winningResult.put(LottoRank.SECOND, winningResult.getOrDefault(LottoRank.SECOND, 0)+1);
            }
            else if(count==5){
                winningResult.put(LottoRank.THIRD, winningResult.getOrDefault(LottoRank.THIRD, 0)+1);
            }
            else if(count==6){
                winningResult.put(LottoRank.FIRST, winningResult.getOrDefault(LottoRank.FIRST, 0)+1);
            }
        }
    ...
    public double getProfitRate(){
        ...
        profitRate = Math.round(totalPrize/amount*100)/100.0;
        return profitRate;
    }
