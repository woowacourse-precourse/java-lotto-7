...
public class LotteryDrawMachine {
    ...
    public LotteryDrawMachine(List<Integer> jackpotNumbers, int bonusNumber) {
        if (jackpotNumbers.size() != LOTTO_NUMBERS_COUNT.getValue() || !jackpotNumbers.stream()
                .allMatch(n -> n >= MIN_NUMBER.getValue() && n <= MAX_NUMBER.getValue())) {
            throw new IllegalArgumentException("[ERROR] 당첨 번호는 1부터 45 사이의 숫자여야 합니다.");
        }
        if (bonusNumber < MIN_NUMBER.getValue() || bonusNumber > MAX_NUMBER.getValue() || jackpotNumbers.contains(
                bonusNumber)) {
