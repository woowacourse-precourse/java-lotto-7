...
public class DefaultLottoMatchChecker implements LottoMatchChecker {
    ...
    private LottoMatchStatus getMatchStatus(int count, boolean bonus) {
        if (count == 3) {
            return LottoMatchStatus.THREE_MATCH_RESULT;
        }

        if (count == 4) {
            return LottoMatchStatus.FOUR_MATCH_RESULT;
        }

        if (bonus && count == 5) {
            return LottoMatchStatus.FIVE_MATCH_WITH_BONUS_RESULT;
        }
        ...
        if (count == 6) {
            return LottoMatchStatus.SIX_MATCH_RESULT;
        }
