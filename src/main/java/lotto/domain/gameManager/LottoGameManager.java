...
public class LottoGameManager implements GameManager {
    ...
    @Override
    public LotteryStatisticsResultDTO checkWinningLottery(int inputAmount,Lotteries boughtLotteries, List<Integer> winningNumbers,
                                                          int bonusNumber,LotteryStatisticsHistory lotteryStatisticsHistory) {
        List<WinningNumberMatchCountVO> winningNumberMatchCounts = boughtLotteries.winningNumberMatchCount(winningNumbers);
    ...
    private Double calculateReturnRate(int inputAmount, BigInteger totalWinningAmount) {
        ...
        double returnRate = totalWinningAmountBigDecimal.divide(BigDecimal.valueOf(inputAmount),3,BigDecimal.ROUND_HALF_UP).doubleValue();
    ...
    private List<Statistics> findWinningLotto(int bonusNumber, List<Statistics> statistics, WinningNumberMatchCountVO vo,
    ...
    private Statistics checkNeedBonusNumber(int bonusNumber, Lottery lottery, Integer matchedNumberCount,
        ...
        if(statistics.getTier().getNeedsBonusNumberMatch()){
            // lottery 번호에 보너스번호가 포함어있고, 필요한 당첨번호와 당첨된번호의 개수가 같은지 확인
    ...
    private Statistics checkBonusMatch(int bonusNumber, Lottery lottery, Integer matchedNumberCount,
        ...
        if(statistics.getTier().getRequiredMatchCount().equals(matchedNumberCount)
                && lottery.getNumbers().contains(bonusNumber)){
            //당첨 개수 증가.
