...
class WinningLottoNumberSelectorTest {
    ...
    public void 로또_번호_매칭_갯수(Lotto lotto,int result){
        //given
        WinningNumber winningNumber = new WinningNumber(Lotto.createWinningRegularLotto("1,2,3,4,5,6"),new BonusNumber("7"));
    ...
    public void 로또_보너스_포함_여부(Lotto lotto,boolean result){
        ...
        //when
        boolean containBonusNumber = winningLottoNumberSelector.containBonusNumber(
                winningLottoNumberSelector.getPurchasedLottos().get(0));

        //then
        assertThat(containBonusNumber).isEqualTo(result);
    }
