...
public class LottoControllerTest {
    ...
    void testCalculateStatistic(){
        double statistic = 0;
        int countHasBonusNumber = 1;
        List<Integer> countMatched = List.of(3, 3, 4, 5, 6);
        HashMap<Integer, Integer> matchedResult = new LinkedHashMap<>();
        ...
        Assertions.assertEquals(2, matchedResult.get(3));
        Assertions.assertEquals(1, matchedResult.get(4));
        Assertions.assertEquals(1, matchedResult.get(5));
        Assertions.assertEquals(1, matchedResult.get(6));
        Assertions.assertEquals(1, countHasBonusNumber);

        statistic += countHasBonusNumber * PrizeInfo.FIVE_BONUS.getPrize();

        statistic = (statistic / 6000) * 100;

        Assertions.assertEquals(33859333.3, Math.round(statistic * 10) / 10.0);
    }
}
