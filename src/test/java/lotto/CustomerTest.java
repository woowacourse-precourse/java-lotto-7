...
class CustomerTest extends NsTest {
    ...
    void buy() {
        assertSimpleTest(() -> {
            run("8000");
            assertThat(customer.buy().size()).isEqualTo(8);
    ...
    void setWinningLotto() {
        assertSimpleTest(() -> {
            run("1,2,3,4,5,6", "7");
            WinningLotto result = customer.setWinningLotto();
    ...
    void countPrizeAndStatistics() {
        assertRandomUniqueNumbersInRangeTest(() -> {
            ...
            Map<Prize, Integer> result = customer.countPrize(customer.buy(), customer.setWinningLotto());
