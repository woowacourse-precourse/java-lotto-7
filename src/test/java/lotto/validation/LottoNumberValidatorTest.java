...
class LottoNumberValidatorTest {
  ...
  void 올바른_로또_번호가_입력되면_예외가_발생하지_않는다() {
    ...
    assertThatCode(() -> LottoNumberValidator.mainValidator(validNumbers))
  ...
  void 로또_번호의_개수가_6개가_아니면_예외가_발생한다() {
    ...
    assertThatThrownBy(() -> LottoNumberValidator.mainValidator(invalidQuantity))
  ...
  void 로또_번호에_범위를_벗어나는_숫자가_포함되면_예외가_발생한다() {
    ...
    assertThatThrownBy(() -> LottoNumberValidator.mainValidator(outOfRangeNumbers))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void 로또_번호에_중복된_숫자가_포함되면_예외가_발생한다() {
    ...
    assertThatThrownBy(() -> LottoNumberValidator.mainValidator(duplicateNumbers))
        .isInstanceOf(IllegalArgumentException.class)
