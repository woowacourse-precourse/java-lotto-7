...
class WinningNumberValidatorTest {
    ...
    @DisplayName("구분자와 숫자 이외의 문자 입력시 IllegalArgumentException 발생")
    @ParameterizedTest
    @ValueSource(strings = {"A", "b", "+", "ㅋ", ":"})
    void 구분자와_숫자_이외의_문자_예외(String input){
    ...
    void 당첨번호_6개_아님_예외(List<Integer> winningNumbers){
        Assertions.assertThatThrownBy(() -> WinningNumberValidator.validateWinningNumbers(winningNumbers))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void 당첨_번호_숫자_중복_예외(int winningNumber){
        List<Integer> winningNumbers = List.of(1,31,41,29,16); // 리스트에 5개의 당첨번호가 들어가있고, 마지막 당첨번호를 넣는 차례라 가정
        Assertions.assertThatThrownBy(() -> WinningNumberValidator.validateWinningNumberrDuplicate(winningNumbers,winningNumber))
                .isInstanceOf(IllegalArgumentException.class)
