    ...
    static Stream<Arguments> 당첨결과데이터() {
        return Stream.of(
                Arguments.of(new Lotto(List.of(1, 2, 3, 4, 5, 6).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.FIRST),
                Arguments.of(new Lotto(List.of(1, 2, 3, 4, 5, 45).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.SECOND),
                Arguments.of(new Lotto(List.of(1, 2, 3, 4, 5, 7).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.THIRD),
                Arguments.of(new Lotto(List.of(1, 2, 3, 4, 8, 9).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.FORTH),
                Arguments.of(new Lotto(List.of(1, 2, 3, 7, 8, 9).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.FIFTH),

                Arguments.of(new Lotto(List.of(11, 12, 13, 17, 18, 19).stream()
                        .map(LottoNum::new).collect(Collectors.toList())), LottoResult.NONE),
