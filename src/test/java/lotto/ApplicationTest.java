...
class ApplicationTest extends NsTest {
    ...
    @Test
    @DisplayName("구입 금액에 따라 로또를 생성하고 출력")
    void generateAndDisplayLottosBasedOnPurchaseAmount() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
    ...
    @Test
    @DisplayName("당첨 결과를 계산하고 출력")
    void displayWinningResultsBasedOnMatchingNumbers() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
    ...
    @Test
    @DisplayName("보너스 번호 포함 5개 일치로 2등 당첨 결과 출력")
    void displaySecondPrizeWinningResultWithBonusNumber() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
                    run("3000", "1,2,3,4,5,6", "7");
                    assertThat(output()).contains(
                            "3개를 구매했습니다.",
                            "[1, 2, 3, 4, 5, 7]", // 5개 + 보너스 일치 (2등)
                            "[8, 21, 23, 41, 42, 43]",
                            "[3, 5, 11, 16, 32, 38]",
                            "3개 일치 (5,000원) - 0개",
                            "4개 일치 (50,000원) - 0개",
    ...
    @Test
    @DisplayName("4개 일치로 4등 당첨 결과 출력")
    void displayFourthPrizeWinningResult() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
                    run("3000", "1,2,3,4,5,6", "7");
                    assertThat(output()).contains(
                            "3개를 구매했습니다.",
                            "[1, 2, 3, 4, 9, 10]", // 4개 일치 (4등)
                            "[8, 21, 23, 41, 42, 43]",
                            "[3, 5, 11, 16, 32, 38]",
                            "3개 일치 (5,000원) - 0개",
                            "4개 일치 (50,000원) - 1개",
    ...
    @Test
    @DisplayName("모두 일치하지 않아 0% 수익률 결과 출력")
    void displayNoMatchingNumbersResult() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
                    run("3000", "1,2,3,4,5,6", "7");
                    assertThat(output()).contains(
                            "3개를 구매했습니다.",
                            "[10, 20, 30, 40, 41, 42]", // 전부 일치하지 않음
                            "[11, 21, 31, 41, 42, 43]",
                            "[12, 22, 32, 42, 43, 44]",
                            "3개 일치 (5,000원) - 0개",
                            "4개 일치 (50,000원) - 0개",
    ...
    @Test
    @DisplayName("6개 일치로 1등 당첨 결과 출력")
    void displayFirstPrizeWinningResult() {
        assertRandomUniqueNumbersInRangeTest(
                () -> {
                    run("3000", "1,2,3,4,5,6", "7");
                    assertThat(output()).contains(
