...
public class LottoGeneratorTest {
    ...
    @DisplayName("생성되는 로또의 번호 수가 " + LottoPrintFormat.NUMBER_COUNT + "개가 아니면 예외가 발생한다.")
    @Test
    void createNumberTest() {
        LottoNumberGenerator lottoNumberGenerator = new LottoNumberGenerator();
        LottoGenerator lottoGenerator = new LottoGenerator(lottoNumberGenerator);

        Assertions.assertEquals(LottoPrintFormat.NUMBER_COUNT, lottoGenerator.createNumbers().size());
    }
}
