...
class WinnerLottoTest {
    ...
    void 입력된_당첨번호_문자열_배열의_크기가_6이_아니면_예외가_발생한다() {
        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5, 6, 7)))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorMessage.LOTTO_NUMBERS_MUST_BE_SIX.getMessage());

        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5)))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorMessage.LOTTO_NUMBERS_MUST_BE_SIX.getMessage());
    }
    ...
    void 숫자_문자열_배열에_숫자_문자열이_범위_내_숫자가_아니면_예외가_발생한다() {
        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5, 0)))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorMessage.LOTTO_NUMBERS_MUST_BE_BETWEEN_1_AND_45.getMessage() + ": 0");

        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5, 46)))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void 입력된_보너스_번호가_범위_내_숫자가_아니면_예외가_발생한다() {
        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5, 6)).setBonusNumber(0))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorMessage.LOTTO_NUMBERS_MUST_BE_BETWEEN_1_AND_45.getMessage() + ": 0");
    }
    ...
    void 입력된_보너스_번호가_당첨번호와_중복되면_예외가_발생한다() {
        assertThatThrownBy(() -> WinnerLotto.from(Arrays.asList(1, 2, 3, 4, 5, 6)).setBonusNumber(6))
                .isInstanceOf(IllegalArgumentException.class)
        ...
        assertThatThrownBy(() -> validator.duplicate(Arrays.asList(1, 2, 3, 4, 5), 5))
                .isInstanceOf(IllegalArgumentException.class)
