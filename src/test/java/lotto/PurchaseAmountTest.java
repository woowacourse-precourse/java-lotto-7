...
class PurchaseAmountTest {
    ...
    void 입력값_1000_나누어떨어지지_않을때() {
        assertThatThrownBy(() -> new PurchaseAmount("1500"))
                ...
                .hasMessage(ErrorCode.PURCHASE_AMOUNT_SHOULD_BE_MULTIPLE_OF_1000.getMessage());
    }
    ...
    void 입력값_양수_아닐때(String money) {
        assertThatThrownBy(() -> new PurchaseAmount(money))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessage(ErrorCode.PURCHASE_AMOUNT_MUST_BE_POSITIVE.getMessage());
    }

    @DisplayName("입력값이 수형식이 아닐 떄 예외처리")
    @ParameterizedTest
    @ValueSource(strings = {"joonil", ":"})
