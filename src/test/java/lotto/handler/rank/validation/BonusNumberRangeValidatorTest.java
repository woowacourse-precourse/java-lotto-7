...
class BonusNumberRangeValidatorTest {
    private BonusNumberRangeValidator validator;
    private HandlerToken handlerToken;

    @BeforeEach
    void 검증객체_핸들러_토큰_초기화() {
        validator = new BonusNumberRangeValidator();
        handlerToken = new HandlerToken();
    }

    @Test
    @DisplayName("보너스 번호가 범위 내일 경우 통과")
    void validate_보너스_번호가_범위_내일_경우() {
    ...
    @Test
    @DisplayName("보너스 번호가 범위를 벗어날 경우 예외 발생")
    void validate_보너스_번호가_범위를_벗어날_경우() {
        WinningNumberDTO winningNumberDTO = WinningNumberDTO.create("1,2,3,4,5,6", "49");
        handlerToken.addContent(TokenType.WINNING_NUMBER_DTO, winningNumberDTO);
