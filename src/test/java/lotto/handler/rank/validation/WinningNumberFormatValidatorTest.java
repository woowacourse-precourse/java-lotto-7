...
class WinningNumberFormatValidatorTest {
    private WinningNumberFormatValidator validator;
    private HandlerToken handlerToken;

    @BeforeEach
    void 검증객체_핸들러_토큰_초기화() {
        validator = new WinningNumberFormatValidator();
        handlerToken = new HandlerToken();
    }

    @Test
    @DisplayName("당첨 번호가 올바른 형식일 경우 통과")
    void validate_당첨_번호가_올바른_형식일_경우() {
    ...
    @Test
    @DisplayName("당첨 번호가 잘못된 형식일 경우 예외 발생")
    void validate_당첨_번호가_잘못된_형식일_경우() {
        WinningNumberDTO winningNumberDTO = WinningNumberDTO.create("1,2,3,4,%,6a", "7");
        handlerToken.addContent(TokenType.WINNING_NUMBER_DTO, winningNumberDTO);
