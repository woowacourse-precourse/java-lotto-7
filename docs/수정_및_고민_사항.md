...
**문제점**
* 입력받는 부분에서 예외가 일어나면 어떻게 다시 입력받을 수 있을까?

**분석**
* try - catch 로 예외가 일어나는 상황을 잡으면서 입력 받으면 될 것 같음.
* 대신 예외가 일어날 때 마다, 계속 입력 받아야 하기 때문에 while(true) 사용하면 됌.

**해결**
* while(true) , try - catch 문 사용

----
## 고민사항

**문제점**
* 컨트롤러 작성하는 과정에서, 원시값들이 컨트롤러 단에 너무 쉽게 노출됌.

**분석**
* 무언가 감싸서 의미를 명확하게 하고 값을 안전하게 보관하고 싶음 -> dto 작성 필요할듯??
* 그럼 dto로 변환은 어디서 해줘야 할까?
* 도메인 로직에서 dto 로 반환하는 것은 view 에 대한 책임을 가지고 있는 거라, 의존하는 것 같음
* 따라서 그냥 매퍼를 만드는게 나을듯
* 컨트롤러 단에서 매퍼를 통해 바로 변환해서 outputView 에 전달
* 에러 처리 메세지도 dto 로 감싸야함.
